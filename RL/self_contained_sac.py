...
from util.observation_wrappers import GoalConcatWrapper
...
class Args:
    ...
    wandb_project_name: str = "ppo-skill-learning"
...
class SoftQNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(
            np.array(env.single_observation_space.shape).prod() + np.prod(env.single_action_space.shape), 200)
...
class Actor(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.single_observation_space.shape).prod(), 200)
        self.fc2 = nn.Linear(200, 200)
        self.fc_mean = nn.Linear(200, np.prod(env.single_action_space.shape))
        self.fc_logstd = nn.Linear(200, np.prod(env.single_action_space.shape))
        # action rescaling
...
"""
    ...
    rb = ReplayBuffer(
        args.buffer_size,
        Box(-np.inf, np.inf, (4,), np.float32),
    ...
    for global_step in range(args.total_timesteps):
        ...
        for idx, item in enumerate(infos):
            if 'TimeLimit.truncated' in item:
                real_next_obs[idx] = item["terminal_observation"]
